...
public class Algorithm {
	...
	// machine i. default: -1
	public int[] forced_partial_assignment = new int[8];
	...
	public int[] best = new int[8]; // store the best solution
	...
	// Constructor
	public Algorithm(int[][] machine_penalty, int[] forced_partial_assignment, int[][] forbidden_machine,
			int[][] too_near_tasks) {
	...
	public void exploreBranch(ArrayList<Task> arr, ArrayList<Task> tasks, int index, Task task) {
		...
		if (DEBUG) {
			...
			for (int i = 0; i < tasks.size(); ++i) {
				System.out.printf("%d ", tasks.get(i));
			}
			System.out.println("\n");
		}
		...
		if (new_arr.size() >= numTasks) {
			...
			if (checkForcedPartialAssignment (new_arr)) {
				if (checkForbiddenMachine (new_arr)) {
					...
					currentPenalty += tooNearPenaltiesCalculator(new_arr);
					
					if (currentPenalty < minPenalty) {
						
						minPenalty = currentPenalty;
						System.arraycopy(new_arr, 0, best, 0, best.length);
					}
				}
			}
	...
	public int tooNearPenaltiesCalculator(ArrayList<Task> arr) {
		...
		for (int i = 0; i < 7;) {
			tmp = too_near_tasks[arr.get(i).id][arr.get(i++).id];
			if (tmp == -1) {
				return -1;
			} else {
				tooNearPenalty += tmp;
		...
		if (too_near_tasks[arr.get(7).id][arr.get(0).id] == -1) {
			return -1;
		} else {
			tooNearPenalty += tmp;
		}
	...
	public boolean checkForcedPartialAssignment(ArrayList<Task> arr) {
		for (int i = 0; i < 8; i++) {
			if (this.forced_partial_assignment[i] != arr.get(i).id && this.forced_partial_assignment[i] != -1) {
				return false;
			}
		}
		return true;
